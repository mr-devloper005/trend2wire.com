...
const mediaTaskAliases = ['mediaDistribution', 'media-distribution', 'media', 'press-release', 'press', 'release', 'news', 'online-media', 'public-relation', 'media-network', 'directory-press', 'business', 'news-agency']

const getPostTypeText = (post: SitePost) => {
  const content = getContent(post)
  const contentType = asText(content.type) || asText(content.task) || asText(content.contentType)
  const tagType = Array.isArray(post.tags) ? post.tags.find((item) => mediaTaskAliases.includes(String(item).trim())) : ''
  return (contentType || tagType || '').trim()
}

const isPublishedPost = (post: SitePost) => {
  const status = typeof (post as unknown as { status?: unknown }).status === 'string'
    ? String((post as unknown as { status?: string }).status).toUpperCase()
    : ''
  return !status || status === 'PUBLISHED'
}

const isMediaDistributionPost = (post: SitePost) => {
  const type = getPostTypeText(post)
  if (!type) return true
  const normalized = type.replace(/_/g, '-').toLowerCase()
  return mediaTaskAliases.map((item) => item.toLowerCase()).includes(normalized)
}

const uniqueRealPosts = (posts: SitePost[]) =>
  Array.from(new Map(posts.filter(isPublishedPost).filter(isMediaDistributionPost).map((post) => [post.slug || post.id || post.title, post])).values())
...
async function fetchEditableMediaDistributionPosts({ page, limit, category }: { page: number; limit: number; category: string }) {
  ...
  const feeds = await Promise.all([
    fetchSiteFeed(limit, { task: 'mediaDistribution', category: categoryParam, page, fresh: true, timeoutMs: 5000 }).catch(() => null),
    fetchSiteFeed(limit, { task: 'media-distribution', category: categoryParam, page, fresh: true, timeoutMs: 5000 }).catch(() => null),
    fetchSiteFeed(limit, { category: categoryParam, page, fresh: true, timeoutMs: 5000 }).catch(() => null),
    fetchSiteFeed(100, { task: 'mediaDistribution', category: categoryParam, fresh: true, timeoutMs: 5000 }).catch(() => null),
    fetchSiteFeed(100, { task: 'media-distribution', category: categoryParam, fresh: true, timeoutMs: 5000 }).catch(() => null),
    fetchSiteFeed(100, { category: categoryParam, fresh: true, timeoutMs: 5000 }).catch(() => null),
  ])

  const posts = uniqueRealPosts(feeds.flatMap((feed) => feed?.posts || []))
  const paged = posts.slice((page - 1) * limit, page * limit)
  const feedPagination = feeds.find((feed) => feed?.pagination && (feed.posts || []).length)?.pagination

  return {
    posts: paged.length ? paged : posts.slice(0, limit),
    pagination: feedPagination && feedPagination.total > 0
      ? feedPagination
      : fallbackPagination(page, limit, posts.length),
  }
}
...
function EditorialArchive({
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#f4f6f8] text-[#111]">
        <section className="border-b border-[var(--editable-border)] bg-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            ...
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-black/65">
              Real published media distribution posts organized for fast discovery, campaign review, and newsroom visibility.
            </p>
          </div>
        </section>

        <section className="border-b border-[var(--editable-border)] bg-[var(--slot4-blue-dark)] text-white">
          ...
          <section className="mx-auto grid max-w-[var(--editable-container)] border-x border-black bg-white lg:grid-cols-[1.75fr_0.75fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden border-b border-black lg:border-b-0 lg:border-r">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-lg bg-[var(--slot4-accent)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em]">{getCategory(lead, label)}</span>
                <h2 className="editorial-serif mt-5 max-w-4xl text-4xl font-black leading-[0.98] tracking-[-0.045em] sm:text-6xl">{lead.title}</h2>
  
              </div>
            </Link>
            <div className="grid">
              <div className="border-b border-black bg-[var(--slot4-blue-dark)] p-6 text-white">
        ...
        <section className="mx-auto max-w-[var(--editable-container)] bg-[#f4f6f8] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b-4 border-[var(--slot4-blue-dark)] pb-4">
            <h2 className="text-4xl font-black sm:text-5xl">Published post cards</h2>
            <form action={basePath} className="flex rounded-lg border border-[var(--editable-border)] bg-white">
              ...
              <button className="h-11 rounded-lg bg-[var(--slot4-accent)] px-5 text-xs font-black uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group editable-card overflow-hidden border border-[var(--editable-border)] bg-white transition hover:-translate-y-1 hover:shadow-xl">
