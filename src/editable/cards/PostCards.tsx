...
export function RailPostCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group block min-h-[240px] rounded-lg border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-5 ${dc.motion.lift}`}>
      <div>
        ...
        <h3 className="mt-5 line-clamp-3 text-2xl font-black leading-tight">{post.title}</h3>
        <p className="mt-4 line-clamp-4 text-sm leading-7 text-black/62">{getEditableExcerpt(post, 170)}</p>
        <span className="mt-5 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-[var(--slot4-blue-dark)]">Open post <ArrowRight className="h-4 w-4" /></span>
      </div>
    </Link>
  )
}
...
export function CompactIndexCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className="group grid min-w-0 grid-cols-[46px_1fr] gap-4 rounded-lg border border-[var(--editable-border)] bg-white p-5 transition hover:-translate-y-1 hover:shadow-lg">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-2 line-clamp-2 text-sm leading-6 text-black/55">{getEditableExcerpt(post, 110)}</p>
      </div>
    </Link>
  )
}
